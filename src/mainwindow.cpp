...
void MainWindow::setupDashboard() {
    ...
    logDisplay->setMaximumHeight(200);
...
void MainWindow::setupInitialData() {
    ...
    for (auto* m : menus) {
        orderMenuSelect->addItem(QString("%1 - %2").arg(QString::fromStdString(m->name))
                                 .arg(formatPrice(m->price)), m->id);
    }
...
void MainWindow::applyStyleSheet() {
    setStyleSheet(R"(
        ...
        QTabWidget::pane { border: 1px solid #DCDCDC; border-radius: 6px; }
        QTabBar::tab { background: #E0E0E0; padding: 8px 16px; border-radius: 4px 4px 0 0; }
        QTabBar::tab:selected { background: #1976D2; color: white; font-weight: bold; }
        QPushButton { padding: 8px 14px; border-radius: 6px; border: none;
                      background: #E0E0E0; font-weight: bold; }
        QPushButton:hover { background: #BDBDBD; }
        QPushButton#btnPrimary { background: #1976D2; color: white; }
        QPushButton#btnPrimary:hover { background: #1565C0; }
        QPushButton#btnDanger { background: #D32F2F; color: white; }
        QPushButton#btnDanger:hover { background: #B71C1C; }
        QPushButton#btnSuccess { background: #388E3C; color: white; }
        QPushButton#btnSuccess:hover { background: #2E7D32; }
        QPushButton#btnWarning { background: #F57C00; color: white; }
        QPushButton#btnWarning:hover { background: #E65100; }
        QGroupBox { font-weight: bold; border: 1px solid #BDBDBD;
        ...
        QTextEdit { border: 1px solid #BDBDBD; border-radius: 4px; font-family: monospace; }
    )");
}
...
void MainWindow::refreshOrderTable() {
    ...
    for (auto it = orders.begin(); it != orders.end(); ++it, ++row) {
        ...
        if (o->status == "pending")   status->setForeground(QColor("#FF9800"));
        if (o->status == "preparing") status->setForeground(QColor("#1976D2"));
        if (o->status == "served")    status->setForeground(QColor("#4CAF50"));
...
void MainWindow::refreshHistoryList() {
    ...
    for (auto& a : actions) {
        historyList->addItem(QString("[%1] %2").arg(QString::fromStdString(a.type))
                             .arg(QString::fromStdString(a.description)));
    }
}
...
void MainWindow::refreshDashboard() {
    ...
    int occupied = 0;
    for (auto* t : tableList->getAll()) if (t->isOccupied) occupied++;
...
void MainWindow::onAddMenuItem() {
    try {
        ...
        // Callback untuk logging (syarat 5)
        forEachMenu({&item}, [this](const MenuItem& m) {
            addLog(QString("➕ Menu ditambah: %1 (%2)")
                   .arg(QString::fromStdString(m.name))
                   .arg(formatPrice(m.price)));
        });
        ...
        // Update combo order
        orderMenuSelect->addItem(QString("%1 - %2").arg(name).arg(formatPrice(price)), item.id);
...
void MainWindow::onSearchMenu() {
    ...
    // Cari by nama (Hash Table)
    MenuItem* byName = menuHash->findByName(query.toStdString());
    if (byName) {
        menuTable->setRowCount(1);
        menuTable->setItem(0, 0, new QTableWidgetItem(QString::number(byName->id)));
        menuTable->setItem(0, 1, new QTableWidgetItem(QString::fromStdString(byName->name)));
        menuTable->setItem(0, 2, new QTableWidgetItem(QString::fromStdString(byName->category)));
        menuTable->setItem(0, 3, new QTableWidgetItem(formatPrice(byName->price)));
        menuTable->setItem(0, 4, new QTableWidgetItem(byName->available ? "✅ Ya" : "❌ Tidak"));
        return;
    }

    // Filter partial match menggunakan lambda (syarat 11)
    auto all = menuList->getAll();
...
void MainWindow::onAddStaff() {
    ...
    addLog(QString("👤 Staf ditambah: %1 (%2)").arg(name).arg(inputStaffRole->currentText()));
...
void MainWindow::onRemoveStaff() {
    ...
    if (ok) {
        ...
        if (duty) {
            duty->onDuty = true;
            lblOnDuty->setText(QString("Sedang Bertugas: %1 (%2)")
                .arg(QString::fromStdString(duty->name))
                .arg(QString::fromStdString(duty->role)));
        } else {
    ...
    try {
        ...
        if (next) {
            lblOnDuty->setText(QString("Sedang Bertugas: %1 (%2)")
                               ...
                               .arg(QString::fromStdString(next->role)));
